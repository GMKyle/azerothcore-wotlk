...
{
    if (me->GetCharmer()->GetTypeId() == TYPEID_UNIT)
...
bool PlayerAI::IsPlayerRangedAttacker(Player const* who)
{
    ...
    switch (who->getClass())
    {
        ...
        /*default:
            sLog->outError("entities.unit", "Possessed player %s (possessed by %s) does not have any recognized class (class = %u).", std::to_string(who->GetGUID()).c_str(), std::to_string(who->GetCharmerGUID()).c_str(), who->getClass());
            break;*/
    }
}
        ...
        case CLASS_ROGUE:
        {
            ...
            switch (GetSpec())
            {
                ...
                case SPEC_ROGUE_SUBLETY:
                    ...
                    VerifyAndPushSpellCast(spells, SPELL_ROG_PREPARATION, TARGET_NONE, 10);
                    if (!me->IsWithinMeleeRange(me->GetVictim()))
